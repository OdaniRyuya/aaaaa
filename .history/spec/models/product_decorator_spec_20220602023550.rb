...
RSpec.describe Potepan::ProductDecorator, type: :model do
  describe "related_products" do
    ...
    let(:products1) { create_list(:product, 4, taxons: [taxon[0]]) }
    let(:products2) { create_list(:product, 4, taxons: [taxon[1]]) }
    let(:duplicate_products) { products1 + related_products }
    let(:duplicate_products) { create_list(:product, 4, taxons: [taxon[0],taxon[1]]) }
    

    context "related_productsに関連商品が含まれている" do
      it "関連商品が含まれていること" do
        expect(related_products).to eq products1
      end

      it "関連商品が重複していないこと" do
        expect(related_products).to eq duplicate_products.uniq
      end
