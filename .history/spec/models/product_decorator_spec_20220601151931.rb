...
RSpec.describe Potepan::ProductDecorator, type: :model do
  describe "related_products" do
    ...
    let!(:duplicate_product) {  create_list(:product, 4, taxons: [taxon1]) } 

    context "related_productsに関連商品が含まれている" do
      it "関連商品が含まれていること" do
      binding.pry
        expect(related_products).to include products
      end

      it "関連商品が重複していないこと" do
      binding.pry
        expect(product.related_products.pluck(:id)).to eq correct_related_products.pluck(:id)
      end
