...
RSpec.describe Potepan::ProductDecorator, type: :model do
  describe "related_products" do
    ...
    let(:no_related_product) { create(:product, taxons: taxon[1].taxonomy_id) }
    let(:main_product) { create(:product, taxons: taxon[0].taxonomy_id) }
    let(:related_products) { main_product.related_products }
    let(:products) { create_list(:product, 4, taxons: taxon[0].taxonomy_id] }
