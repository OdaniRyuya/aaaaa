...
RSpec.describe Potepan::ProductDecorator, type: :model do
  describe "related_products" do
    ...
    let!(:duplicate_related_products) do
      Spree::Product.in_taxons(main_product.taxons)
    end
    let(:duplicate_products) do
      related_products + duplicate_related_products
    end
    ...
    context "related_productsに関連商品が含まれている" do
      ...
      it "関連商品が重複していないこと" do
        binding.pry
        expect(related_products.pluck(:id)).to_not eq duplicate_related_products.pluck(:id)
        expect(related_products.pluck(:id)).to_not eq duplicate_products.pluck(:id).distinct
      end
