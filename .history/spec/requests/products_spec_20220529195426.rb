...
RSpec.describe 'Products', type: :request do
  ...
  describe "商品詳細ページ" do
    ...
    it "関連商品が取得できるか" do
      ...
      expect(response.body).to include product_list.name
      expect(response.body).to include product_list.display_price.to_s
      end
    end
    
    scenario "関連商品から個別のページへ移動できる" do
      click_on product_lists.first.name
      expect(current_path).to eq potepan_product_path(product_lists.first.id)
    end
  end
end
