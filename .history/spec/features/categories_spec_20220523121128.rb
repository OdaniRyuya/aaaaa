...
RSpec.feature "CategoriesFeatures", type: :feature do
  ...
  describe "capカテゴリに関すること" do
    before { visit potepan_category_path(cap.id) }

    scenario "capページの表示が正しいこと" do
    expect(page).to have_title "#{cap.name} - BIGBAG Store"
    expect(page).to have_selector ".page-title h2", text: "cap"
    ...
    scenario "カテゴリリンクから正しいページに移動すること" do
      click_on cap.name
      expect(current_path).to eq potepan_category_path(cap.id)
      expect(page).to have_selector ".productCaption h5", text: cap.name
      expect(page).to have_selector ".productCaption h3", text: cap.display_price
      expect(page).not_to have_selector ".productCaption h5", text: product.name
      expect(page).not_to have_selector ".productCaption h3", text: product.display_price
    end
  end
