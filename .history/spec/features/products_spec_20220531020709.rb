...
RSpec.feature "ProductsFeatures", type: :feature do
  ...
  let(:related_product) { create(:product, name: "relation-test1", taxons: [taxon]) }
  ...
  describe "タクソノミーに関すること" do
    ...
    scenario "関連商品のリンクが正しく遷移できること" do
      click_link "relation-test1"
      visit potepan_product_path(related_product.id)
      expect(page).to eq potepan_product_path(product.taxons.first.id)
    end
  end
end
